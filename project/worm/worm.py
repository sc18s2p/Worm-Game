...
class Worm:
    ...
    def _init_spaces(self):
        ...
        self.W = FunctionSpace(mesh, MixedElement([P1_3, P1_3, P1_3, P1, P0, P0, P0]))
    ...
    def _compute_initial_values(self):
        ...
        F0_kappa = (
            dot(kappa_trial, kappa_test) * mu0 * dxL
        ...
        v = TestFunction(Q)
        F_gamma0 = (gamma - dot(grad(e1_n), e2_n) / mu0) * v * dx()
        ...
        fa.assign(
            u_n,
            [x0, Function(V3), kappa0, Function(V), Function(Q), gamma0, Function(Q)],
        )
    ...
    def _update_solution(self):
        ...
        if self.F_op is None:
            # Variational form
            F_x = (
                1.0 / dt * dot(KK * (x - x_n), phi_x) * mu * dx
            ...
            F_w = (
                inner(grad(x), grad(phi_kappa)) / mu * dx
            ...
            F_z = (z - C * (gamma) - D / dt * (gamma - gamma_n)) * phi_z * mu * dx()

            F_gamma = (
                1.0 / dt * (gamma - gamma_n) * phi_gamma * mu * dx()
                - grad(m) * phi_gamma * dx()
                + dot(tau_cross_kappa, 1.0 / dt * grad(x - x_n)) * phi_gamma * dx()
            )
            ...
            # self.F = F
            self.F_op, self.F_rhs = lhs(F), rhs(F)

        F_z_forcing = C * my_gamma * phi_z * mu * dx()
        F_y_forcing = (
            dot(A * (my_alpha * self.e1_n + my_beta * self.e2_n), phi_y) * mu * dx
        )
        F_forcing = F_y_forcing + F_z_forcing
        L = self.F_rhs + rhs(F_forcing)

        # boundary conditions
        kappa_space = self.W.split()[2]
        # kappa_space = self.W.sub(2)
        kappa_b = project(my_alpha * self.e1_n + my_beta * self.e2_n, V3)
        bc = DirichletBC(kappa_space, kappa_b, lambda x, o: o)

        # compute solution
        u = Function(self.W)
        solve(self.F_op == L, u, bcs=bc)
        ...
        def rotated_frame(v):
            ...
            tmp = v * c + cross(k, v) + dot(v, k) / (1 + c) * k
            ret = (
                tmp * cos(varphi)
    ...
    def _to_ndarray(self, var: Function) -> np.ndarray:
        ...
        if fs.num_sub_spaces() == 0:
            arr = np.array(
                [var.vector()[dof_maps[i]] for i in range(dof_maps.shape[0])]
            )
        else:
            arr = np.array(
                [
                    [var.vector()[dof_maps[d][i]] for d in range(dof_maps.shape[0])]
                    for i in range(dof_maps.shape[1])
                ]
            )
        return arr
...
def _sim_test_worm(worm, alpha_pref=None, beta_pref=None, gamma_pref=None, t=2.0):
    ...
    while worm.t < t:
        print(f"t={worm.t:.2f}")
        worm.update(alpha_pref=alpha_pref, beta_pref=beta_pref, gamma_pref=gamma_pref)
...
def test_numpy_forcing():
    ...
    _sim_test_worm(
        worm, alpha_pref=alpha_pref, beta_pref=beta_pref, gamma_pref=gamma_pref
    )
    _print_output(worm)
...
def test_expression_forcing():
    ...
    gamma_pref = Expression("v", degree=0, v=0)
